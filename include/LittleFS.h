/*
 ...
 (find original version in "framework-arduinoespressif8266\libraries\LittleFS\src")
...
// Not clear if and how it works in original implementation on the device.
// At compilation of lfs.c to value is picked from lfs.h. But at compilation of LittleFS.cpp the value is picked from here.
// So far this redefinition causes in tests in a segmentation fault, because the in LittleFS module in uses the value from here,
// while the implementation in in lfs module works with the value from lfs.h.
// Anyway, remove the redefinion here.
...
public:
    ...
    //Mock
    //bool begin() override {
    bool begin(int argc, char **argv) override {
        if (argc >= 2 ) {
